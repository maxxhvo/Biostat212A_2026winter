...
### TODO: remove unecessary variables, if any
```
...
Carseats <- Carseats %>%
  mutate(Sales_Level = as.factor(ifelse(Sales <= 8, "Low", "High"))) %>%
#  select(-Sales) # remove the original Sales variable
```
...
class_tree_fit %>%
  ...
  geom_point() + 
  geom_line() + 
  labs(x = "cost_complexity", y = "CV accuracy")
```
